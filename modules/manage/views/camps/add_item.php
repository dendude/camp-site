...
<tr class="<?= $class ?>">
    ...
    <td>
        <div class="form-group-item m-b-5">
            <?= Html::activeDropDownList($model_item, '[{index}]comission_type', \app\models\CampsContract::getComissionTypes(), [
                'class' => 'form-control input-sm',
            ]) ?>
            <?= $help_block ?>
        </div>
        <div class="form-group-item">
            <?= Html::activeTextInput($model_item, '[{index}]comission_value', [
                'class' => 'form-control input-sm',
                'maxlength' => true,
    ...
    <td>
        <div class="form-group-item m-b-5">
            <?= Html::activeDropDownList($model_item, '[{index}]discount_type', \app\models\CampsContract::getComissionTypes(), [
                'class' => 'form-control input-sm',
            ]) ?>
            <?= $help_block ?>
        </div>
        <div class="form-group-item">
            <?= Html::activeTextInput($model_item, '[{index}]discount_value', [
                'class' => 'form-control input-sm',
                'maxlength' => true,
                'placeholder' => 'Цена'
            ]) ?>
    ...
    <td>
        <div class="form-group-item">
            <?= Html::activeDropDownList($model_item, '[{index}]status', Statuses::statuses(Statuses::TYPE_CAMP), [
                'class' => 'form-control input-sm',
                'value' => ($model_item->isNewRecord ? Statuses::STATUS_ACTIVE : $model_item->status)
            ]) ?>
            <?= $help_block ?>
        </div>
    </td>
